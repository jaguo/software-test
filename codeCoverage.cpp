                   ...
                   BPatch_Vector<BPatch_snippet *> *registerCalls)
{
    ...
    {
        unsigned long startAddress = (*iter)->getStartAddress();
        unsigned long endAddress = (*iter)->getLastInsnAddress();
        cout << "Instrumenting Basic Block 0x" << hex << startAddress 
                << "--0x" << endAddress << " of " << funcName << endl;
        BPatch_Vector<BPatch_snippet *> instArgs;
        ...
        if (NULL == bbEntry)
        {
            cerr << "Failed to find entry for basic block at 0x" << hex << startAddress
                 << endl;
            return false;
        }
        ...
        if (!handle)
        {
            cerr << "Failed to insert instrumention in basic block at 0x" << hex << startAddress << endl;
            return false;
        }
        ...
        BPatch_constExpr startAddrArg(startAddress);
        regArgs.push_back(&startAddrArg);
        BPatch_constExpr endAddrArg(endAddress);
        regArgs.push_back(&endAddrArg);
